...
public class PullToRefreshContainer extends ViewGroup {
    ...
    public PullToRefreshContainer(Context context, @Nullable AttributeSet attrs) {
        this(context, attrs, 0);
    }

    public PullToRefreshContainer(Context context, @Nullable AttributeSet attrs, int defStyleAttr) {
    ...
    private void initAttr(AttributeSet attrs) {
        ...
        //获取自定义属性
        TypedArray ta = context.obtainStyledAttributes(attrs, R.styleable.PullToRefreshContainer);
        if (ta.hasValue(R.styleable.PullToRefreshContainer_pullType)) {
            int type_int = ta.getInt(R.styleable.PullToRefreshContainer_pullType, 0);
            type = Type.values()[type_int];
        }
        if (ta.hasValue(R.styleable.PullToRefreshContainer_give)) {
            int give_int = ta.getInt(R.styleable.PullToRefreshContainer_give, 0);
            give = Give.values()[give_int];
        }
        if (ta.hasValue(R.styleable.PullToRefreshContainer_header)) {
            headerResoureId = ta.getResourceId(R.styleable.PullToRefreshContainer_header, 0);
        }
        if (ta.hasValue(R.styleable.PullToRefreshContainer_footer)) {
            footerResoureId = ta.getResourceId(R.styleable.PullToRefreshContainer_footer, 0);
        }
        ta.recycle();
    }
